...
class TwixtGame:
    ...
    def successor_func(self, move):
        ...
        elif not new_game.get_valid_moves():
            new_game.game_over = True
        else:
    ...
    def evaluate_player(self, player):
        ...
        if player == PLAYER_1:
            for start_y in range(BOARD_SIZE):
                if self.board[0][start_y] == PLAYER_1:
                    for end_y in range(BOARD_SIZE):
                        if self.board[BOARD_SIZE-1][end_y] == PLAYER_1:
                            distance = self.find_connection_distance(0, start_y, BOARD_SIZE-1, end_y, player)
                            if distance != -1:
            ...
            for start_x in range(BOARD_SIZE):
                if self.board[start_x][0] == PLAYER_2:
                    for end_x in range(BOARD_SIZE):
                        if self.board[end_x][BOARD_SIZE-1] == PLAYER_2:
                            distance = self.find_connection_distance(start_x, 0, end_x, BOARD_SIZE-1, player)
                            if distance != -1:
                                score += (BOARD_SIZE - distance) * 10
    ...
    def count_connection(self, player):
        ...
        for i in range(BOARD_SIZE):
            for j in range(BOARD_SIZE):
                if self.board[i][j] == player and (i, j) not in visited:
                    for dx, dy in DIRECTIONS:
                        ni, nj = i + dx, j + dy
    ...
    def play(self):
        ...
        self.print_board()
        if self.winner:
            print(f"{Fore.RED if self.winner == PLAYER_1 else Fore.BLUE}player {self.winner} wins!{Style.RESET_ALL}")
        else:
